...
public sealed partial class UnlockableRulesTreeBuilderScript : Node
{
	...
	{
		string filePath = @"C:\git\alfa_e_betto\Data\acentuaÃ§Ã£o\acentos_dados.json";
		...
		new DiactricalMarkRuleSetItemResource()
		{
			...
			{
				CategoryType = CategoryType.Acentuation,
				Category = "AcentuaÃ§Ã£o grÃ¡fica",
				RuleSetType = c.Type,
				RuleSet = c.Name,
				RuleType = sc.Type,
				Rule = sc.Name,
	...
	{
		string filePath = @"C:\git\alfa_e_betto\Data\acentuaÃ§Ã£o\meteor_words_data.json";
		string jsonString = File.ReadAllText(filePath);
