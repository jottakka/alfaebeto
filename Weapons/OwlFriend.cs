...
public sealed partial class OwlFriend : CharacterBody2D
{
	...
	{
		if (!IsInstanceValid(this)) return; // Check if Owl is still valid
	...
	{
		if (!IsInstanceValid(this) || !IsInstanceValid(area2D)) return;
	...
	{
		if (OscillationPeriodMs <= 0) return;
	...
	{
		if (!IsInstanceValid(this) || !IsInstanceValid(area)) return;
	...
	{
		if (!IsInstanceValid(this) || HasTarget || !IsInstanceValid(area)) return;
