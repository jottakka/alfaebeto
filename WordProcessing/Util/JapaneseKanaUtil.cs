...
public static class JapaneseKanaUtil
{
    ...
    {
        var ruleIdx = _random.Next(ValidRuleTypes.Count());
    ...
    {
        (var hintPool, var optionsPool) = ruleType switch
        {
    ...
    {
        var shuffledItems = new HashSet<(string Option, string Hint)>();
        ...
        var answerIdx = _random.Next(shuffledItems.Count);
        var answer = shuffledItems.ElementAt(answerIdx).Hint;
        var optionsArr = shuffledItems.Select(x => x.Option).ToArray();
